...
@section('content')
    <!--=============================
        BREADCRUMB START
    ==============================-->
    <section class="fp__breadcrumb" style="background: url({{ asset('frontend/frontend-template/images/counter_bg.jpg') }});">
...
{{--                                        <span>{{ @$product->options->product_size['name'] }}--}}
{{--                                            ({{ currencyPosition(@$product->options->product_size['price'] ? '(' . currencyPosition(@$product->options->product_size['price']) . ')' : '' }}</span>--}}
{{--                                        @foreach($product->options->product_options as $option)--}}
{{--                                            <p>{{ $option['name'] }} ({{ currencyPosition($option['price']) }})</p>--}}
...
{{--                                    </td>--}}
                                ...
                                @if(Cart::content()->count() == 0)
                                    <tr>
                                        <td colspan="6" class="text-center p__pro_select" style="width: 100%; display: inline;">
                                            Cart is empty!
                                        </td>
                                    </tr>
    ...
    <script>
        ...
        {{--$('.decrement').on('click', function() {--}}
            ...
            function couponApply(code, subtotal) {
                $.ajax({
                    method: 'POST',
                    url: '{{ route("frontend.apply-coupon") }}',
                    data: {
                        code: code,
                        subtotal: subtotal
                    },
            ...
            function destroyCoupon(){
                $.ajax({
                    method: 'GET',
                    url: '{{ route("frontend.destroy-coupon") }}',
                    beforeSend: function(){
                        showLoader();
                    },
                    success: function(response){
...
{{--                            </div>--}}
{{--                        </div>`--}}
{{--                        $('.coupon_card').html($couponCartHrml);--}}
{{--                        toastr.success(response.message);--}}
{{--                    },--}}
{{--                    error: function(xhr, status, error){--}}
{{--                        let errorMessage = xhr.responseJSON.message;--}}
